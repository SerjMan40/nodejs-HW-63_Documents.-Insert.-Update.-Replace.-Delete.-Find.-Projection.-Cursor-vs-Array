...
export const postArticleHandler = async (req, res) => {
  ...
  try {
    ...
    if (Array.isArray(articles)) {
      ...
      const validArticles = articles.map(item => {
        return {
          title: item.title || 'No title',
          content: item.content || 'No content'
        }
      })
      addedArticles = await addArticleToDB(validArticles)
      if (!addedArticles) {
        throw new Error('Failed to add articles to the database.')
      }
      res.status(201).send(`Articles created successfully with IDs: ${addedArticles.map(article => article._id).join(', ')}`)
    } else {
      const newArticle = { title: articles.title, content: articles.content }
